use std::{fs, str::FromStr, os::unix::thread::RawPthread, collections::HashSet};
...
impl Assingment {
    fn fully_containing(&self, other : &Self) -> bool {
        ...
        let clause_a: HashSet<usize, _> = HashSet::from_iter((*left_other..*right_other).collect::<Vec<usize>>());
	let clause_b: HashSet<usize, _>= HashSet::from_iter((*left_self..*right_self).collect::<Vec<usize>>());
        // let clause_b = (left_other <= left_self) | (right_other >= right_self);
        // println!("{clause} {left_self} {right_self} {left_other} {right_other}");        
         clause_a.intersection(&clause_b);
	// count != 0
	true
    }
}
